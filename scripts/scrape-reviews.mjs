...
async function main() {
  ...
  try {
    ...
    const reviews = await page.evaluate(() => {
      ...
      // Also collect all reviewer avatar images (they're 36x36 from lh3.googleusercontent.com/a-/)
      const allAvatars = Array.from(document.querySelectorAll('img[src*="googleusercontent.com/a-"]'));
      
      for (let idx = 0; idx < shareBtns.length; idx++) {
        const btn = shareBtns[idx];
        ...
        for (let i = 0; i < 15; i++) {
          ...
          if (textEl) {
            ...
            // Get avatar - walk up from share button and find nearest avatar image
            let avatarUrl = '';
            let parent = btn;
            for (let j = 0; j < 10; j++) {
              ...
              if (avatarImg) {
                // Get high-res version by replacing size params
                const src = avatarImg.getAttribute('src') || '';
                avatarUrl = src.replace(/=w\d+-h\d+/, '=w200-h200');
                break;
              }
            }
    ...
    const referencedAvatars = new Set(reviews.map(r => r.avatarLocal).filter(l => l.startsWith('/images/avatars/')).map(l => l.split('/').pop()));
